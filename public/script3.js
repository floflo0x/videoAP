    globalThis.document.addEventListener('DOMContentLoaded', () => {
      ...
      movieButton.addEventListener('click', async (event) => {
        ...
        if (movieData.hasOwnProperty('success') == false && movieData['belongs_to_collection'] !== null) {
            ...
            return `
              <div class="row border mb-4">
                ...
                <div class="col-12 col-sm-12 col-md-12 mt-2">
                  <p class="text-center">Upload Link</p>
                </div>
                <div class="col-12 col-sm-12 col-md-12 mt-2 mb-2 d-flex justify-content-between border-0">
                  <input type="url" id="homepage_${movieParts.id}" name="fileUrl"
                    placeholder="Paste your Link here...."
                    style="width: 70%; border: none; border-bottom: 1px solid;"
                  >
                  <button type="button" class="btn upload-link-btn border-0" data-link-id="${movieParts.id}">
                    <i class="fa-solid fa-cloud-arrow-up"></i>
                  </button>
                </div>
                <!-- subtitle section -->
          ...
          episodeList.innerHTML = `
            <div class="row border mb-4">
              ...
              <div class="col-12 col-sm-12 col-md-12 mt-2 mb-2 d-flex justify-content-between border-0">
                <input type="url" id="homepage_${movieData.id}" name="fileUrl"
                  placeholder="Paste your Link here...."
                  style="width: 70%; border: none; border-bottom: 1px solid;"
                >
                <button type="button" class="btn upload-link-btn border-0" data-link-id="${movieData.id}">
                  <i class="fa-solid fa-cloud-arrow-up"></i>
                </button>
              </div>
              <!-- subtitle section -->
        ...
        globalThis.document.querySelectorAll('.upload-link-btn').forEach(button => {
          button.addEventListener('click', function(e) {
            const movieShowId = this.getAttribute('data-link-id');
            const inputUrl = globalThis.document.getElementById(`homepage_${movieShowId}`).value.trim();
            // const button = document.querySelector(`button[data-movieshow-id="${movieShowId}"]`);
            // button.disabled = true;
            // console.log(movieShowId, e.target, inputUrl);
            // console.log(button);
            uploadLink(movieShowId, inputUrl, this);
          });
        });
        ...
        function uploadLink(movieShowId, inputUrl, btn) {
          submitButton.style.display = 'none';
          btn.disabled = true;
          ...
          if (inputUrl === '') {
            alert('Please insert a link to upload.');
            btn.disabled = false;
            ...
            // console.log(isValidUrl(inputUrl), button);
          ...
          const fileInput = document.querySelector(`#episodeFile_${movieShowId}`);
          const videoUploader = document.querySelector(`#video-uploader_${movieShowId}`);

          // console.log("poppy jack blue....");

          const fCode = document.getElementById(`homepage_${movieShowId}`);

          const spinner = document.getElementById(`spinner_${movieShowId}`);
          // spinner.style.display = 'block';

          fCode.value = inputUrl;
        ...
        function uploadSubVideo(tvShowId, btn) {
          ...
          fetch("https://mateys.xyz/h4kig_api/connection/upload_subtitle.php",requestOptions)
            .then(response => response.json())
            .then(async (result) => {
              // console.log(result);
          ...
          if (seasonData && seasonData.length >= 1) {
            const resultsHTML = seasonData.map((tvShow) => {
              return `<div class="row border mb-4">
                ...
                <div class="col-12 col-sm-12 col-md-12 mt-2 mb-2 d-flex justify-content-between border-0">
                  <input type="url" id="homepage_${tvShow.id}" name="fileUrl"
                    placeholder="Paste your Link here...."
                    style="width: 70%; border: none; border-bottom: 1px solid;"
                  >
                  <button type="button" class="btn upload-link-btn border-0" data-link-id="${tvShow.id}">
                    <i class="fa-solid fa-cloud-arrow-up"></i>
                  </button>
                </div>
                <!-- subtitle section -->
            ...
            globalThis.document.querySelectorAll('.upload-link-btn').forEach(button => {
              button.addEventListener('click', function(e) {
                const tvShowId = this.getAttribute('data-link-id');
                const inputUrl = globalThis.document.getElementById(`homepage_${tvShowId}`).value.trim();
                // const button = document.querySelector(`button[data-tvshow-id="${tvShowId}"]`);
                // button.disabled = true;
                // console.log(tvShowId, e.target, inputUrl);
                uploadLink(tvShowId, inputUrl, this);
              });
            });
            ...
            function uploadLink(tvShowId, inputUrl, btn) {
              submitButton.style.display = 'none';
              btn.disabled = true;
              // console.log(inputUrl);

              const button = document.querySelector(`button[data-movieshow-id="${tvShowId}"]`);
              button.disabled = true;
              // console.log(button);
              ...
              const fCode = document.getElementById(`homepage_${tvShowId}`);
